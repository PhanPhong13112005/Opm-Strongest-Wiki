...
const SpineFigure = defineAsyncComponent(() => import('./SpineFigure.vue'))
...
const spineFailed = ref(false)
const normalizeName = value => String(value || '').normalize('NFD').replace(/[\u0300-\u036f]/g, '').trim().toLowerCase()
const featuredSpine = computed(() => {
  const characterId = String(featuredItem.value?.id || '').split('-')[0]
  if (!characterId || characterId === 'unknown') return null

  return backgearChibis.find(character => character.heid === characterId)
    || backgearChibis.find(character => normalizeName(character.name) === normalizeName(featuredName.value))
    || null
})
watch(() => featuredItem.value?.id, () => {
  spineReady.value = false
  spineFailed.value = false
})
const handleSpineLoaded = ({ animated }) => {
  spineReady.value = animated
  spineFailed.value = !animated
}
const handleSpineFail = () => {
  spineReady.value = false
  spineFailed.value = true
}
    ...
    <section class="release-hero" :style="{ '--hero-accent': getTypeAccent(featuredItem) }">
      ...
      >
        ...
        <img
          :src="featuredImage"
          :alt="featuredName"
          :class="{ 'is-spine-replaced': featuredSpine && spineReady && !spineFailed }"
          onerror="this.style.display='none'"
        />
        <div
          v-if="featuredSpine && !spineFailed"
          class="release-hero__spine"
          :class="{ 'is-ready': spineReady }"
          aria-hidden="true"
        >
          <SpineFigure :src="featuredSpine.json" @loaded="handleSpineLoaded" @fail="handleSpineFail" />
        </div>
...
.fade-enter-active,.fade-leave-active,.slide-left-enter-active,.slide-left-leave-active,.slide-right-enter-active,.slide-right-leave-active{transition:opacity .38s ease,transform .38s cubic-bezier(.2,.8,.2,1)}
.fade-enter-from,.fade-leave-to{opacity:0;transform:translateY(8px)}
.slide-left-enter-from{opacity:0;transform:translateX(34px)}.slide-left-leave-to{opacity:0;transform:translateX(-34px)}
.slide-right-enter-from{opacity:0;transform:translateX(-34px)}.slide-right-leave-to{opacity:0;transform:translateX(34px)}@keyframes ambientDrift{to{transform:translate(80px,70px) scale(1.15)}}@keyframes pulseDot{50%{opacity:.35;transform:scale(.75)}}@keyframes heroFloat{50%{transform:translateY(-10px) scale(1.01)}}@keyframes glowPulse{50%{opacity:.55;transform:scale(1.12)}}@keyframes slowSpin{to{transform:rotate(360deg)}}@keyframes heroScan{0%,55%{transform:translateX(-120%)}85%,100%{transform:translateX(120%)}}@keyframes cardReveal{from{opacity:0;transform:translateY(22px)}to{opacity:1;transform:translateY(0)}}@keyframes cardShine{to{transform:translateX(130%)}}
...
@media(prefers-reduced-motion:reduce){.ambient,.hero-pill i,.release-hero__visual img,.energy-ring,.hero-scan,.card-light,.release-card{animation:none!important}.release-card,.release-card__image img,.fade-enter-active,.fade-leave-active,.slide-left-enter-active,.slide-left-leave-active,.slide-right-enter-active,.slide-right-leave-active{transition:none!important}}
</style>
