...
3.  **Tải Dữ Liệu:** Trang Nhân vật, Sự kiện và Lịch ra mắt ưu tiên API công khai đọc từ Neon,
    có cache ngắn hạn ở client/CDN và tự dùng JSON trong `/data` khi API lỗi. Các module chưa chuyển
    đổi vẫn đọc JSON trực tiếp và render theo locale hiện tại (`vi` hoặc `en`).
