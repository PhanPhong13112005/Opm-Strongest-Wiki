...
import { createTopUp, getMyTopUps, updateCouponTopUp } from '../services/communityApi'

const router = useRouter()

const packages = [
  { id: 'coupon-6', coupons: 6, price: 13000, label: 'Gói 6 Coupon' },
]
...
const error = ref('')
const notice = ref('')
const pendingCouponRequest = ref(null)
const pendingCouponRequestKey = 'opmwiki.coupon.pending-request'

const readPendingCouponRequest = () => {
  if (pendingCouponRequest.value) return pendingCouponRequest.value
  try {
    pendingCouponRequest.value = JSON.parse(sessionStorage.getItem(pendingCouponRequestKey) || 'null')
  } catch {
    sessionStorage.removeItem(pendingCouponRequestKey)
  }
  return pendingCouponRequest.value
}

const savePendingCouponRequest = value => {
  pendingCouponRequest.value = value
  sessionStorage.setItem(pendingCouponRequestKey, JSON.stringify(value))
}

const clearPendingCouponRequest = () => {
  pendingCouponRequest.value = null
  sessionStorage.removeItem(pendingCouponRequestKey)
}

const createOrderCode = () => {
  const value = globalThis.crypto?.randomUUID?.()
  if (!value) throw new Error('Trình duyệt không hỗ trợ tạo mã đơn an toàn.')
  return value.replaceAll('-', '').toUpperCase()
}

const resolveOrderCode = fingerprint => {
  const pending = readPendingCouponRequest()
  if (pending?.fingerprint === fingerprint && /^[A-Z0-9]+$/.test(pending.orderCode || '')) {
    return pending.orderCode
  }
  const orderCode = createOrderCode()
  savePendingCouponRequest({ fingerprint, orderCode })
  return orderCode
}
...
const parseCouponOrder = (referenceCode = '') => {
  const match = /^UID:([^|]+)\|SID:([^|]+)\|CP:(\d+)\|QTY:(\d+)\|(.+)$/.exec(referenceCode)
  return match ? {
    uid: match[1],
    serverId: match[2],
    coupons: Number(match[3]) * Number(match[4]),
    orderCode: match[5],
  } : null
}
...
const load = async () => {
  ...
  try {
    ...
    orders.value = requests
      .filter(item => item.provider === 'Coupon Order')
      .map(item => ({ ...item, coupon: parseCouponOrder(item.referenceCode) }))
  } catch (exception) {
...
const isCancellable = order => ['Pending', 'PaymentReported'].includes(order.status)

const cancelOrder = async order => {
  if (!globalThis.confirm(`Bạn muốn hủy đơn Coupon #${order.id}?`)) return

  cancellingId.value = order.id
  error.value = ''
  notice.value = ''
  try {
    await updateCouponTopUp(order.id, 'cancel')
    notice.value = `Đã hủy đơn Coupon #${order.id}.`
    await load()
  } catch (exception) {
    error.value = exception.message
  } finally {
...
const normalizeProfile = () => {
  const match = uid.value.trim().match(/^(\d{5,20})_(\d{1,20})$/)
  if (!match) return
  uid.value = match[1]
  if (!server.value.trim()) server.value = match[2]
}

const submit = async () => {
  if (!hasValidSession()) {
    await router.push({ name: 'login', query: { redirect: '/top-up' } })
    return
  }
  ...
  try {
    const fingerprint = [
      ...
      cleanUid,
      cleanServer,
      selectedPackage.value.coupons,
      quantity.value,
      totalPrice.value,
    ].join('|')
    const orderCode = resolveOrderCode(fingerprint)
    ...
    notice.value = `Đã gửi yêu cầu nạp ${totalCoupons.value} Coupon. Quản trị viên sẽ kiểm tra và xử lý.`
    await load()
  } catch (exception) {
    error.value = exception.message
  } finally {
  ...
  <main class="coupon-page">
    <div class="coupon-shell">
      <header class="coupon-heading">
        <div>
          <p>CỬA HÀNG NẠP THẺ</p>
          <h1>Nạp One Punch Man: The Strongest</h1>
          <span>Nhập UID, Server và chọn gói Coupon cần nạp vào tài khoản game.</span>
        </div>
        <div class="coupon-heading__badge">
          <b>{{ isAuthenticated ? 'CP' : 'ID' }}</b>
          <span>{{ isAuthenticated ? `Số dư: ${formatMoney(authState.session?.balance || 0)}` : 'Xem gói trước, đăng nhập khi đặt hàng' }}</span>
        </div>
      </header>
      ...
      <form class="coupon-order" novalidate @submit.prevent="submit">
          ...
          <div class="coupon-profile__fields">
            <label>
              ...
              <input
                ...
                maxlength="41"
                required
                placeholder="VD: 15253771_310765"
                @blur="normalizeProfile"
              />
            </label>
            <label>
        ...
        <section class="coupon-summary">
          ...
          <button :disabled="submitting">
            {{ submitting
              ? 'Đang gửi yêu cầu…'
              : `${isAuthenticated ? 'Đặt hàng' : 'Đăng nhập để đặt hàng'} · ${formatMoney(totalPrice)}` }}
            <span>→</span>
          </button>
        </section>
      </form>

      <section v-if="isAuthenticated" class="coupon-history">
        <header>
          <div><span>LỊCH SỬ</span><h2>Yêu cầu Coupon của bạn</h2></div>
          <div class="coupon-history__tools">
            <b>{{ orders.length }} yêu cầu</b>
            <button type="button" :disabled="loading" @click="load">{{ loading ? 'Đang tải…' : 'Cập nhật' }}</button>
          </div>
        </header>
        ...
        <div v-else-if="orders.length" class="coupon-history__list">
          <article v-for="order in orders" :key="order.id">
            <div class="coupon-history__info">
              <strong>#{{ order.id }} · {{ formatMoney(order.amount) }}</strong>
              <span v-if="order.coupon">UID {{ order.coupon.uid }} · Server {{ order.coupon.serverId }} · {{ order.coupon.coupons }} Coupon</span>
              <span v-else>{{ order.referenceCode }}</span>
              <p v-if="order.staffNote" class="coupon-history__note"><b>Phản hồi quản trị:</b> {{ order.staffNote }}</p>
            </div>
            <div class="coupon-history__status">
              <b :class="`is-${order.status.toLowerCase()}`">{{ statusLabel(order.status) }}</b>
              <time>{{ order.reviewedAt ? 'Xử lý' : 'Gửi' }} lúc {{ formatDate(order.reviewedAt || order.createdAt) }}</time>
              <button
                v-if="isCancellable(order)"
                type="button"
                class="coupon-history__cancel"
      ...
      <section v-else class="coupon-history coupon-history--guest">
        <header>
          <div><span>LỊCH SỬ</span><h2>Đăng nhập để xem đơn hàng của bạn</h2></div>
          <RouterLink :to="{ name: 'login', query: { redirect: '/top-up' } }">Đăng nhập</RouterLink>
        </header>
      </section>
